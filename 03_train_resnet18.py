...
# Load pretrained ReNet 18
# weights = "IMAGENET1K_V1" loads weights pretrained on ImageNet, whihc is the standard starting point for transfer learning
model = models.resnet18(weights = "IMAGENET1K_V1")

# Resnet18 final layer outputs 1000 classess. We need to replace it with a new layer that outputs 7 classes instead matching the skin lesion categories
num_features = model.fc.in_features # Size of the input to final layer
model.fc = nn.Linear(num_features, num_classes)
#move the model to the GPU or CPU
model = model.to(device)
print(model.fc) # should show linear(in_features = 512, outfeatures = 7)
